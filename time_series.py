...
#then plot certain selected years in colour
        ...
        xax = range(1,13)
        plt.plot(xax,sample_yr_1,color="DarkGray",linewidth=1)
        plt.plot(xax,sample_yr_2,color="DarkGray",linewidth=1)
        plt.plot(xax,sample_yr_3,color="DarkGray",linewidth=1)
        plt.plot(xax,sample_yr_4,color="DarkGray",linewidth=1)
        plt.plot(xax,sample_yr_5,color="DarkGray",linewidth=1)
       
        years_of_interest = [2010,2005,1998,2014,2015]
        colours_of_int = ['FireBrick','SteelBlue','Pink','Gold','Green']
        i = 0
        ...
        plt.axis((0,13,0.21,0.95))
        plt.show()

    def plot_scenario(self,title):
#plot scenarios for 2015
        ytd = self.pull_year_to_date_with_fill(2015)
        xax = range(1,13)

        #loop over 5 different "scenarios" for remainder of year
        for i in range(0,5):
            ytd = self.pull_year_to_date_with_fill(2015,i)
            print title,i,ytd[11]
            plt.plot(xax,ytd,color="Black",linewidth=2)

        years_of_interest = [2010,2005,1998,2013,2003,2002,2007,2006,2009,2014,2015]
        colours_of_int = ['FireBrick','SteelBlue','DarkGray','DarkGray','DarkGray','DarkGray',
                          'DarkGray','DarkGray','DarkGray','Gold','Green']
        i=0
        for y in years_of_interest:
        ...
        if title == "Cowtan and Way Hybrid" or title=="ERA":
            ...
            plt.axis((0,16,0.09,0.55))
        else:
            plt.axis((0,16,0.37,0.80))
    ...
    def predict(self,nmonths):

        series1 = self.annualise(nmonths)
        series2 = self.annualise()

        diffs = []

        for i in range(0,len(series1.times)):
            diffs.append((series1.data[i]-series2.data[i])**2.)
...
class time_series:
    ...
    @staticmethod
    def make_ar_series(ar, trend, sd):
        times = np.arange(1881.0,2016.0,1.0)
        data = [random.gauss(0.0,sd)]
        lounc = [data[0]-0.1]
        hiunc = [data[0]+0.1]
        for i in range(1,len(times)):
            data.append(ar*data[i-1] + random.gauss(0.0,sd))
            lounc.append(data[i]-0.1)
            hiunc.append(data[i]+0.1)

        for i in range(0,len(times)):
            data[i] += trend.data[i]
            lounc[i] += trend.data[i]
            hiunc[i] += trend.data[i]
            
        f = time_series(times,data,lounc,hiunc)
...
#                print self.times[i-filter_width+1:i+1]
    ...
    def modded_skyscraper_diagram(self):
        ...
        plt.axis((1997.5,2018.5,-0.2,0.8))
        plt.show()
    ...
    def plot_ts_highlight_nino(self,ninos):
        ...
        plt.xlabel('Year', fontsize=18)
        plt.ylabel('Anomaly relative to 1961-1990 (K)', fontsize=18)
        plt.axis((1949.5,2016.5,-0.32,0.77))
        plt.show()

    def plot_ts_only_ninos(self,ninos,cols):

        delta = 0.5

        #plt.plot(self.times,self.data,color="DimGray")
        ...
        for i in range(0,len(self.times)):
            ...
            if y > 1950:
                ...
                if nino_year(y) == -1:
                    ynina.append(y)
                    dnina.append(d)
                if y in [1982,1997,2015]:
                    ybig.append(y)
                    dbig.append(d)

        plt.plot(yneut,dneut,'o',markersize=9,color=cols[1],markeredgecolor='none')
        plt.plot(ynino,dnino,'o',markersize=9,color=cols[0],markeredgecolor='none')
        plt.plot(ynina,dnina,'o',markersize=9,color=cols[2],markeredgecolor='none')

        plt.plot(ybig,dbig,'D',markersize=11,color='FireBrick',markeredgecolor='none')
        
        plt.xlabel('Year', fontsize=18)
        plt.ylabel('Anomaly relative to 1961-1990 (K)', fontsize=18)
        plt.axis((1949.5,2016.5,-0.42,0.83))
        plt.show()
...
#draw off-screen polygons to get an appropriate legend
        ...
        plt.plot([1949.5,2016.5],[0,0],color="Black")
        plt.axis((1949.5,2016.5,-0.32,0.82))
...
#add year lables to each coloured bar
    ...
    def plot_bis_ranking_diagram(self,iters=1000):
...
#ranks to display (2.5%, 25%, 50%, 75% and 97.5%)
        ...
        for yr in range(y1,y2+1):
            ...
            rk = self.get_rank_of_year(yr)

            if yr >= 1850 and yr <=1910:
                col = colscheme[7]
            elif yr >= 1911 and yr <=1930:
                col = colscheme[6]
            elif yr >= 1931 and yr <=1950:
                col = colscheme[5]
            elif yr >= 1951 and yr <=1970:
                col = colscheme[4]
            elif yr >= 1971 and yr <=1990:
                col = colscheme[3]
            elif yr >= 1991 and yr <=2000:
                col = colscheme[2]
            ...
            poly = Polygon(zip([rk-barthick,rk+barthick,rk+barthick,rk-barthick],\
                               [lolo,lolo,hihi,hihi]), facecolor=col, alpha=0.5,\
                           edgecolor="None")
            plt.gca(zorder=2).add_patch(poly)
            poly = Polygon(zip([rk-barthick,rk+barthick,rk+barthick,rk-barthick],\
                               [lo,lo,hi,hi]), facecolor=col,\
                           edgecolor="None")
            plt.gca(zorder=2).add_patch(poly)
#            plt.plot([rk-0.5,rk+0.5],[md,md],color="Black")
            plt.plot([rk-0.5,rk+0.5],[-1*rk,-1*rk],color="Black")

#add year lables to each coloured bar
            if rk < 51:
                plt.text(rk-0.2,lolo+5,str(int(yr)),fontsize=10,rotation=90)


            print yr,int(round(mode(ranks)[0][0])),\
                  int(np.percentile(ranks,2.5)),\
                  int(np.percentile(ranks,25)),\
                  int(np.percentile(ranks,50)),\
                  int(np.percentile(ranks,75)),\
                  int(np.percentile(ranks,97.5))


#draw some off-screen polygons to get an appropriate legend
        poly = Polygon(zip([0,0,0,0],[0,0,0,0]),facecolor=colscheme[0],edgecolor="None",label="2011-2014")
        plt.gca().add_patch(poly)
        poly = Polygon(zip([0,0,0,0],[0,0,0,0]),facecolor=colscheme[1],edgecolor="None",label="2001-2010")
        plt.gca().add_patch(poly)
        poly = Polygon(zip([0,0,0,0],[0,0,0,0]),facecolor=colscheme[2],edgecolor="None",label="1991-2000")
        plt.gca().add_patch(poly)
        poly = Polygon(zip([0,0,0,0],[0,0,0,0]),facecolor=colscheme[3],edgecolor="None",label="1971-1990")
        plt.gca().add_patch(poly)
        poly = Polygon(zip([0,0,0,0],[0,0,0,0]),facecolor=colscheme[4],edgecolor="None",label="1951-1970")
        plt.gca().add_patch(poly)
        poly = Polygon(zip([0,0,0,0],[0,0,0,0]),facecolor=colscheme[5],edgecolor="None",label="1931-1950")
        plt.gca().add_patch(poly)
        poly = Polygon(zip([0,0,0,0],[0,0,0,0]),facecolor=colscheme[6],edgecolor="None",label="1911-1930")
        plt.gca().add_patch(poly)
        poly = Polygon(zip([0,0,0,0],[0,0,0,0]),facecolor=colscheme[7],edgecolor="None",label="1850-1910")
        plt.gca().add_patch(poly)

        plt.legend(loc='best')

        plt.yticks([-1, -5, -10, -20, -30, -40, -50, -60], ['1', '5', '10','20','30','40','50','60'])
        plt.xticks([1, 5, 10, 20, 30, 40, 50], ['1', '5', '10','20','30','40','50'])

        plt.xlabel('Nominal rank', fontsize=18)
        plt.ylabel('Rank', fontsize=18)
        plt.axis((0, 50.5, -60,8))
        plt.show()


    def plot_yr_vs_prob_of_rank(self,iters=1000):
        ...
        p1[1,0] = 0.5
        plt.imshow(p1, cmap='RdBu_r')
        cols = plt.get_cmap('RdBu_r')
        ...
        for yr in range(y1,y2+1):
            ...
            for i in range(0,nranks):
                p = float(ranks.count(i))/float(iters)
                poly = Polygon(zip([yr-0.5,yr+0.5,yr+0.5,yr-0.5],\
                                   [-1*(i-0.5),-1*(i-0.5),-1*(i+0.5),-1*(i+0.5)])\
                               ,facecolor=cols(p/0.5),edgecolor="None")
                plt.gca().add_patch(poly)


        plt.yticks([-1, -5, -10, -20, -30, -40, -50, -60, -80, -100,-120],\
                   ['1', '5', '10','20','30','40','50','60','80','100','120'])
        #plt.xticks([1, 5, 10, 20, 30, 40, 50], ['1', '5', '10','20','30','40','50'])

        plt.colorbar(orientation='vertical')
        plt.axis((y1-0.5,y2+0.5, -1*nranks-0.5, -0.5 ))
        plt.ylabel('Probability of this rank...', fontsize=18)
        plt.xlabel('... for a given year', fontsize=18)
        plt.show()


    def p_y1_gt_y2(self,y1,y2):

        ya = y1 #- min(self.times)
        yb = y2 #- min(self.times)

        diff = self.get_value(ya)-self.get_value(yb)
        unc1 = (self.hiunc[self.times.index(ya)]-self.lounc[self.times.index(ya)])/3.92
        unc2 = (self.hiunc[self.times.index(yb)]-self.lounc[self.times.index(yb)])/3.92
        unc = np.sqrt(unc1**2 + unc2**2)
        p = phi(diff/unc)

        return p

    def plot_relative_rank_matrix(self):
        ...
        for ya in range(y1,y2):
            for yb in range(ya+1,y2+1):
                diff = self.get_value(ya)-self.get_value(yb)
                unc1 = (self.hiunc[self.times.index(ya)]-self.lounc[self.times.index(ya)])/3.92
                unc2 = (self.hiunc[self.times.index(yb)]-self.lounc[self.times.index(yb)])/3.92
                unc = np.sqrt(unc1**2 + unc2**2)
                ...
                poly = Polygon(zip([ya-0.5,ya+0.5,ya+0.5,ya-0.5],[yb-0.5,yb-0.5,yb+0.5,yb+0.5]),facecolor=cols(1-p),edgecolor="None")
                plt.gca().add_patch(poly)
                poly = Polygon(zip([yb-0.5,yb+0.5,yb+0.5,yb-0.5],[ya-0.5,ya-0.5,ya+0.5,ya+0.5]),facecolor=cols(p),edgecolor="None")
                plt.gca().add_patch(poly)

        
        plt.colorbar(orientation='vertical')
        plt.axis((y1-0.5,y2+0.5, y1-0.5, y2+0.5 ))
        plt.xlabel('...than this year', fontsize=18)
        plt.ylabel('Probability this year is warmer...', fontsize=18)
        plt.show()

    def get_rank_percentiles(self,year,percentiles,iters=1000):

        ranks = []
        ct = 0
        for i in range(0,iters):
            g = self.draw_sample().get_rank_of_year(year)
            if g == 1:
                ct += 1
            ranks.append(g)

        out_ranks_at_percentiles = []
        for p in percentiles:
            out_ranks_at_percentiles.append(int(np.percentile(ranks,p)))

        print "P warmest",float(ct)/float(iters)

        return out_ranks_at_percentiles
...
#ranks to display (2.5%, 25%, 50%, 75% and 97.5%)
        ...
        for yr in range(y1,y2+1):
            ...
            print yr,int(round(mode(ranks)[0][0])),\
                  int(np.percentile(ranks,2.5)),\
                  int(np.percentile(ranks,10)),\
                  int(np.percentile(ranks,25)),\
                  int(np.percentile(ranks,50)),\
                  int(np.percentile(ranks,75)),\
                  int(np.percentile(ranks,90)),\
                  int(np.percentile(ranks,97.5))
...
def nino_year(year):
    ...
    elninos = [1958, 1966, 1973, 1983, 1987, 1988, 1998, 2003, 2010, 2016]
